...
socket.on('iniciar_cuenta_regresiva', (letra) => {
    ...
    msgElem.innerHTML = `Letra elegida: <br><span style="font-size: 3rem; font-weight: bold; color: #ff007f;">${letra}</span><br> Preparados, listos...`;
    timerElem.innerHTML = count;
...
function startGameWithLetter(letter) {
    reiniciarContadorSugerencias(); // <--- AGREGA ESTA LÍNEA AQUÍ
...
function processResultsAndShow() {
    ...
    currentCategories.forEach(cat => {
        ...
        // Verificamos si la palabra existe en el diccionario inteligente
        const esValidaEnDiccionario = validarConDiccionario(cat.id, selectedLetter, cleanVal);

        if (esValidaEnDiccionario) {
            ...
            // Si empieza con la letra pero no está en el diccionario por ahora
            evaluatedResults.push({ category: cat.name, word: inputVal, status: 'incorrect', points: 0, textBadge: 'No en diccionario' });
        }
        }
    });
...
let categoriaActualSugerencia = '';
let letraActualSugerencia = '';

function abrirModalSugerencia(categoria, letra, palabraActual) {
    categoriaActualSugerencia = categoria;
    letraActualSugerencia = letra;
    
    // CORRECCIÓN AQUÍ: Separamos el ID del .innerText
    document.getElementById('suggestInfoText').innerText = `Categoría: ${categoria} | Letra: ${letra}`;
    document.getElementById('suggestWordInput').value = palabraActual || '';
...
function enviarSugerenciaServidor() {
    const palabraSugerida = document.getElementById('suggestWordInput').value.trim();
    if (!palabraSugerida) return;

    socket.emit('sugerir_palabra', {
        categoria: categoriaActualSugerencia,
        letra: letraActualSugerencia,
        palabra: palabraSugerida.toUpperCase()
    });

    alert("¡Gracias! Tu sugerencia ha sido enviada al creador.");
    cerrarModalSugerencia();
}

let contadorSugerenciasPartida = 0;
const MAX_SUGERENCIAS_PERMITIDAS = 3; // Límite máximo por partida

function abrirModalSugerenciaPartida() {
    // Verificamos si ya alcanzó el límite de envíos
    if (contadorSugerenciasPartida >= MAX_SUGERENCIAS_PERMITIDAS) {
        alert("Has alcanzado el límite de 3 sugerencias para esta partida. ¡Gracias por colaborar!");
        return;
    }

    // Rellenar dinámicamente el selector con las categorías de la partida actual
    const selectCat = document.getElementById('selectSugerenciaCat');
    selectCat.innerHTML = '';
    
    const categoriasActuales = difficultyCategories[currentDifficulty];
    ...
    // Fijar automáticamente la letra que tocó en la ruleta de la partida
    document.getElementById('inputSugerenciaLetra').value = selectedLetter || 'A';
    document.getElementById('inputSugerenciaPalabra').value = '';

    // Mostrar cuántas sugerencias le quedan disponibles
    const restan = MAX_SUGERENCIAS_PERMITIDAS - contadorSugerenciasPartida;
    document.getElementById('contadorSugerencias').innerText = `Te quedan ${restan} sugerencia(s) disponible(s) en esta partida.`;

    // Mostrar el modal
    document.getElementById('modalSugerenciaPartida').style.display = 'flex';
}

function cerrarModalSugerenciaPartida() {
    document.getElementById('modalSugerenciaPartida').style.display = 'none';
}

function enviarSugerenciaRestringida() {
    const categoria = document.getElementById('selectSugerenciaCat').value;
    const letra = selectedLetter;
    const palabra = document.getElementById('inputSugerenciaPalabra').value.trim();

    if (!palabra) {
        alert("Por favor, escribe una palabra antes de enviar.");
        return;
    }

    // Emitir la sugerencia al servidor mediante Socket.io
    socket.emit('sugerir_palabra', {
        categoria: categoria,
        letra: letra,
    ...
    contadorSugerenciasPartida++;
    alert("¡Sugerencia enviada con éxito! Gracias por ayudar a crecer el diccionario.");
    cerrarModalSugerenciaPartida();
}

// Opcional: Reiniciar el contador cuando inicie una nueva partida
function reiniciarContadorSugerencias() {
    contadorSugerenciasPartida = 0;
}